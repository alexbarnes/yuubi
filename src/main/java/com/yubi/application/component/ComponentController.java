...
public class ComponentController {
	...
	@Inject
	public ComponentController(ComponentAccess componentAccess,
			SupplierAccess supplierAccess, ComponentService componentService) {
		super();
	...
	public Model edit(@PathVariable("id") long id) {
		Model model = new Model(ScreenMode.UPDATE, "component", "component",
				componentAccess.load(id));
		return model;
	}

	@RequestMapping("/view/{id}")
	public Model view(@PathVariable(value = "id") final long id) {
		Model model = new Model(ScreenMode.ENQUIRE, "component", "component",
				componentAccess.load(id));
	...
	public Model save(@ModelAttribute("component") @Valid Component component,
		...
		if (result.hasErrors()) {
			return new Model(ScreenMode.UPDATE, "component", "component",
					component);
		}
		...
		if (component.getSupplierId() > 0)
			component
					.setSupplier(supplierAccess.load(component.getSupplierId()));
	...
	public Model updateStock(
			@ModelAttribute("stockupdate") @Valid StockUpdate update,
			BindingResult result) {

		Component component = componentService.loadComponentWithHistory(update
				.getComponentId());

		if (result.hasErrors()) {
			Model model = new Model(ScreenMode.ENQUIRE, "component",
					"component", component);
			model.addObject("stockupdate", update);
	...
	public @ResponseBody
	List<ComponentDTO> search(String query) {
		String[] queryStrings = { "*", query, "*" };
		List<Component> components = componentService.search(StringUtils
				.join(queryStrings));
		List<ComponentDTO> results = new ArrayList<ComponentDTO>();

		for (Component component : components) {
			results.add(new ComponentDTO(component.getId(), component
					.getDescription()));
		}
