		...
		<c:forEach items="${product.components}" var="current" varStatus="status">
			<tr>
				<td>
					...
					<c:if test="${readOnly}">
						<span class="input-xlarge uneditable-input">${product.components[status.index].id.component.description}</span>
					</c:if>
					<form:input
						path="product.components[${status.index}].componentId"
						type="hidden" id="component${status.index}" />
					</td>
				<td>
					<c:if test="${not readOnly}">
						<form:input
							path="product.components[${status.index}].number" class="input-small"/>
					</c:if>
					<c:if test="${readOnly}">
					<span class="input-small uneditable-input">${product.components[status.index].number}</span>	
					</c:if>
				</td>
				<c:if test="${not readOnly}">
					<td>
						<a  role="button" 
							...
							onclick="removeComponent(${status.index});">Remove</a>
					</td>
				</c:if>
			</tr>
		</c:forEach>
